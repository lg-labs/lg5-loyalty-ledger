...
/**
 * Round-trip + REQ-013 + REQ-004 verification for the {@code movement} ledger
 * via the {@link MovementLedgerRepository} output port.
 *
 * <p>
 * Specifically asserts:
 * <ul>
 * <li>A credit and a debit can be appended and read back with every field
 * intact (mapper round-trip preserves {@code originatingEventId},
 * REQ-014).</li>
 * <li>The Postgres {@code loyalty_cause} ENUM round-trips correctly through
 * {@link jakarta.persistence.EnumType#STRING} mapping (relies on the JDBC URL's
 * {@code stringtype=unspecified}).</li>
 * <li>The Spring Data repository surface declares only {@code save(...)} and
 * read methods — no {@code update*} or {@code delete*} (REQ-013).</li>
 * <li>{@code existsCreditFor} (REQ-004) returns {@code true} only when a
 * positive-delta movement exists for that order.</li>
 * </ul>
 */
class MovementLedgerRepositoryIT extends Bootstrap {
	...
	void credit_movement_round_trips_through_the_port() {
		...
		final Movement saved = this.movementLedgerRepository
				.save(Movement.ofCredit(customerId, orderId, eventId, "OrderPaidEvent", receivedAt, 100));

		final Movement found = this.movementLedgerRepository.findById(saved.getId()).orElseThrow();
		...
		// Compare instants — Postgres timestamptz normalizes the zone (typically to
		// UTC),
	...
	void debit_movement_with_cancelled_cause_round_trips() {
		...
		final Movement saved = this.movementLedgerRepository
				.save(Movement.ofDebit(customerId, orderId, UUID.randomUUID(), "OrderCancelledEvent",
						ZonedDateTime.now(), -50, BalanceUpdateCause.ORDER_CANCELLED));
	...
	void existsCreditFor_returns_true_only_when_a_prior_credit_exists() {
		...
		this.movementLedgerRepository.save(Movement.ofCredit(customer, orderWithCredit, UUID.randomUUID(),
				"OrderPaidEvent", ZonedDateTime.now(), 200));

		assertThat(this.movementLedgerRepository.existsCreditFor(orderWithCredit)).isTrue();
		assertThat(this.movementLedgerRepository.existsCreditFor(orderWithoutCredit)).isFalse();
	}

	@Test
	void existsCreditFor_is_false_when_only_a_debit_exists_for_the_order() {
		// Defensive: a debit alone (delta < 0) must NOT satisfy "credit exists"
		// (REQ-004).
		final OrderId orderId = OrderId.random();
		this.movementLedgerRepository.save(Movement.ofDebit(CustomerId.random(), orderId, UUID.randomUUID(),
				"OrderRefundedEvent", ZonedDateTime.now(), -30, BalanceUpdateCause.ORDER_REFUNDED));

		assertThat(this.movementLedgerRepository.existsCreditFor(orderId)).isFalse();
	}
	...
	void jpa_repository_does_not_expose_update_or_delete_methods() {
		...
		for (final Method m : declared) {
			assertThat(m.getName()).as("MovementJpaRepository must not expose mutating method '" + m.getName() + "'")
