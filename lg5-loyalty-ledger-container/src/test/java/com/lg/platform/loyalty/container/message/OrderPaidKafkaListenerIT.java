 ...
 *
 * <p>
 * Brings up a Postgres testcontainer (full app context boots Liquibase + JPA
 * wiring) plus a Confluent Kafka + Schema Registry container pair (RULE-013
 * gates flipped via {@code @TestPropertySource}). The application-service input
 * port is replaced with a Mockito bean because the real handler ships in
 * TASK-011.
 *
 * <p>
 * The bootstrap-servers and schema-registry-url come from the framework's
 * {@code ConfluentKafkaContainerCustomConfig} which publishes them into the
 * Spring {@code Environment} as {@code kafka-config.bootstrap-servers} and
...
class OrderPaidKafkaListenerIT extends Bootstrap {
	...
	void receive_invokes_input_port_exactly_once_with_mapped_command() throws Exception {
		...
		final OrderPaidAvroModel msg = OrderPaidAvroModel.newBuilder().setMessageId(eventId).setCustomerId(customerId)
				.setOrderId(orderId).setPaidAmount(new BigDecimal("12.95"))
				.setCreatedAt(Instant.parse("2026-05-10T05:30:00Z")).build();

		try (final KafkaProducer<String, Object> producer = this.newAvroProducer()) {
			producer.send(new ProducerRecord<>(this.orderPaidTopic, customerId.toString(), msg)).get();
			producer.flush();
		}
