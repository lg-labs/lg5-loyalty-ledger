...
/**
 * REST IT for {@code GET /loyalty/customers/{customerId}/movements} (TASK-016,
 * REQ-010 + REQ-013).
 *
 * <p>
 * Seeds 60 movements for one customer with strictly distinct
 * {@code appended_at} timestamps so the page boundary tests can rely on the
 * {@code DESC} ordering being unambiguous. The {@link MovementJpaRepository} is
 * autowired directly (rather than the {@code MovementLedgerRepository} output
 * port) because {@code Movement.ofCredit/ofDebit} hard-codes
 * {@code appendedAt =
 * ZonedDateTime.now()} which would collide at sub-millisecond resolution under
 * a tight loop. Using the JPA entity straight gives the test full control over
 * the timestamp axis without weakening the production write path.
 *
 * <p>
 * Postgres-only — same shape as {@link CustomerBalanceControllerIT}; the
 * message layer is not involved.
 */
class CustomerMovementsControllerIT extends Bootstrap {
	...
	void getMovements_three_pages_cover_all_60_in_descending_order_no_overlap() {
		...
		final List<MovementJpaEntity> seeded = this.seedMovements(customerId, N);

		// Sanity: the seed succeeded.
		assertThat(this.movementJpaRepository.countByCustomerId(customerId.getValue())).isEqualTo(N);

		final JsonPath p0 = this.fetchPage(customerId, 0, PAGE_SIZE);
		final JsonPath p1 = this.fetchPage(customerId, 1, PAGE_SIZE);
		final JsonPath p2 = this.fetchPage(customerId, 2, PAGE_SIZE);
        ...
        this.assertDescendingByAppendedAt(p0);
        this.assertDescendingByAppendedAt(p1);
        this.assertDescendingByAppendedAt(p2);

		// The boundary between pages is also descending: last of page N >= first of
		// page N+1.
		assertThat(this.lastAppendedAt(p0)).isAfterOrEqualTo(this.firstAppendedAt(p1));
		assertThat(this.lastAppendedAt(p1)).isAfterOrEqualTo(this.firstAppendedAt(p2));
		...
		final ZonedDateTime newestSeed = seeded.stream().map(MovementJpaEntity::getAppendedAt)
				.max(ZonedDateTime::compareTo).orElseThrow().truncatedTo(ChronoUnit.MICROS);
		assertThat(this.firstAppendedAt(p0).truncatedTo(ChronoUnit.MICROS)).isEqualTo(newestSeed);
	}

	@Test
	void getMovements_out_of_range_page_returns_200_with_empty_movements_and_correct_total() {
		final CustomerId customerId = CustomerId.random();
        this.seedMovements(customerId, N);
		...
		RestAssured.given(this.requestSpecification).filter(OpenApiContractFilter.openApiValidator())
				...
				.get("/loyalty/customers/{customerId}/movements?page=999&size=20", customerId.getValue().toString())
				.then().statusCode(200).contentType("application/vnd.api.v1+json").body("page", equalTo(999))
				.body("size", equalTo(20)).body("totalElements", equalTo(N)).body("movements", hasSize(0));
	}
	...
	void getMovements_unknown_customer_returns_200_with_empty_array_and_zero_total() {
		...
		assertThat(this.movementJpaRepository.countByCustomerId(unknown)).isZero();

		RestAssured.given(this.requestSpecification).filter(OpenApiContractFilter.openApiValidator())
				.accept("application/vnd.api.v1+json").when()
				.get("/loyalty/customers/{customerId}/movements", unknown.toString()).then().statusCode(200)
				.contentType("application/vnd.api.v1+json").body("totalElements", equalTo(0))
	...
	/**
	 * Inserts {@code n} movements for {@code customerId} with strictly distinct
	 * {@code appended_at} timestamps spaced 1 ms apart, all causes =
	 * {@code ORDER_PAID}, all deltas = +1. Returns the inserted entities.
	 */
	private List<MovementJpaEntity> seedMovements(final CustomerId customerId, final int n) {
		...
		return java.util.stream.IntStream.range(0, n)
				.mapToObj(i -> MovementJpaEntity.builder().id(UUID.randomUUID()).customerId(customerId.getValue())
						.delta(1).cause(BalanceUpdateCause.ORDER_PAID).originatingOrderId(UUID.randomUUID())
						.originatingEventId(UUID.randomUUID()).originatingEventType("OrderPaid")
						.originatingEventReceivedAt(base.plusNanos(i * 1_000_000L))
						.appendedAt(base.plusNanos(i * 1_000_000L)).version(0).build())
				.map(this.movementJpaRepository::save).toList();
	}

	private JsonPath fetchPage(final CustomerId customerId, final int page, final int size) {
		return RestAssured.given(this.requestSpecification).accept("application/vnd.api.v1+json").when()
				.get("/loyalty/customers/{customerId}/movements?page={page}&size={size}",
						customerId.getValue().toString(), page, size)
				.then().statusCode(200).contentType("application/vnd.api.v1+json").extract().jsonPath();
	}
	...
	private void assertDescendingByAppendedAt(final JsonPath page) {
		...
		for (int i = 1; i < appendedAts.size(); i++) {
			...
			assertThat(prev).as("appendedAt at index %d must be >= index %d", i - 1, i).isAfterOrEqualTo(curr);
		}
	}
