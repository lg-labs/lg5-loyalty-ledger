...
/**
 * IT for
 * {@link com.lg.platform.loyalty.message.listener.kafka.OrderRefundedKafkaListener}.
 *
 * <p>
 * Mirrors {@link OrderPaidKafkaListenerIT} and
 * {@link OrderCancelledKafkaListenerIT}: same Postgres + Kafka containers, same
...
class OrderRefundedKafkaListenerIT extends Bootstrap {
	...
	void receive_invokes_input_port_exactly_once_with_mapped_command() throws Exception {
		...
		final OrderRefundedAvroModel msg = OrderRefundedAvroModel.newBuilder().setMessageId(eventId)
		...
		try (final KafkaProducer<String, Object> producer = this.newAvroProducer()) {
			producer.send(new ProducerRecord<>(this.orderRefundedTopic, customerId.toString(), msg)).get();
			producer.flush();
		}
