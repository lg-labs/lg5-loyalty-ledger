 ...
 *
 * <p>
 * Each {@code *ContainerCustomConfig} is gated by
 * {@code testcontainers.<name>.enabled} (RULE-013); the ITs flip the relevant
 * property via {@code @TestPropertySource}. Including a config here whose gate
 * is {@code false} is a no-op (the {@code @ConditionalOnProperty} prevents bean
 * registration), so this loader is safe to share across data-access ITs
