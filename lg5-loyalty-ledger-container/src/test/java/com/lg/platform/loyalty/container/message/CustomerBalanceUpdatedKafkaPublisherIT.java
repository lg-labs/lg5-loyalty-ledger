...
/**
 * IT for {@link com.lg.platform.loyalty.message.publisher.kafka.CustomerBalanceUpdatedKafkaPublisher}
 * driven by {@link com.lg.platform.loyalty.application.outbox.scheduler.CustomerBalanceUpdatedOutboxScheduler}
 * (TASK-013).
 *
 * <p>Arrange: insert a {@code STARTED} outbox row carrying a JSON
 * {@link CustomerBalanceUpdatedEventPayload}.
 *
 * <p>Act: rely on the always-on test scheduler (200ms tick — overridden
 * in the shared {@code @TestPropertySource} block below) to pick the
 * row up and publish.
 *
 * <p>Assert (1) a {@link CustomerBalanceUpdatedAvroModel} arrives on
 * the outbound topic with field-by-field 1:1 translation including
 * the outbox-row UUID flowing into the Avro {@code messageId}, and
 * (2) the outbox row is marked {@code COMPLETED} via the producer
 * callback path.
 *
 * <p><b>Shared {@code @TestPropertySource} block.</b> All Kafka
 * container ITs in this package (this publisher IT plus the three
 * listener ITs) declare the <em>identical</em> property set so they
 * share one Spring TestContext + one set of testcontainers across
 * the IT phase. The scheduler is left always-on at 200ms in tests:
 * harmless in listener ITs (the input port is {@code @MockitoBean}
 * → no outbox rows ever written → scheduler iterates an empty list),
 * required here. See OrderPaidKafkaListenerIT for the network-alias
 * collision history that motivates the byte-for-byte unification.
 */
@Slf4j
...
class CustomerBalanceUpdatedKafkaPublisherIT extends Bootstrap {
    ...
    void cleanOutbox() {
        ...
        outboxRepository.deleteAllByStatus(OutboxStatus.STARTED);
        outboxRepository.deleteAllByStatus(OutboxStatus.COMPLETED);
        outboxRepository.deleteAllByStatus(OutboxStatus.FAILED);
    }
    ...
    void scheduler_picks_up_STARTED_row_publishes_avro_to_kafka_and_marks_COMPLETED() throws Exception {
        ...
        final CustomerBalanceUpdatedEventPayload payload = new CustomerBalanceUpdatedEventPayload(
                ...
                orderId, originatingEventId, "OrderPaid", occurredAt);

        final String json = objectMapper.writeValueAsString(payload);
        final OutboxMessage saved = outboxRepository.save(
                OutboxMessage.started(originatingEventId, "CustomerBalanceUpdated", json));
        ...
        try (final KafkaConsumer<String, Object> consumer = newAvroConsumer()) {
            consumer.subscribe(List.of(outboundTopic));

            // ── Assert (1): Avro arrives ───────────────────────────
            final CustomerBalanceUpdatedAvroModel record = await()
        ...
        await().atMost(15, TimeUnit.SECONDS).untilAsserted(() -> {
            ...
            assertThat(stillStarted)
                    .as("outbox row should have transitioned out of STARTED")
